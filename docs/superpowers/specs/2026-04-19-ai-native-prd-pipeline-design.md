...
2. AI auto-sets `status: done` on func-req files after tickets are created.
...
| `/jira-tickets` | Auto-sets `done` on func-req files. |
...
|---|---|---|
| **AI** | Generates all artifacts · Challenges all decisions | Makes final decisions · Sets `approved` status |
...
| **Eng Lead** | Validates technical accuracy · Approves artifacts | Writes first drafts · Manually sequences pipeline steps |
...
| Eng Lead PM time | ~1 hour | ~45 min |
